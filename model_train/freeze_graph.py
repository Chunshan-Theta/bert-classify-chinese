...
def init_predict_var(path):
    ...
    if os.path.exists(label2id_file):
        with open(label2id_file, 'rb') as rf:
            ...
            count_labels = len(label2id.items())
        print('count_labels:%d' % count_labels)
    else:
        print('Can\'t found %s' % label2id_file)
    return count_labels, label2id, id2label
...
def optimize_class_model(args, logger=None):
    ...
    try:
        ...
        #增加 从label2id.pkl中读取count_labels, 这样也可以不用指定count_labels参数； 2019/4/17
        if not args.count_labels:
            count_labels, label2id, id2label = init_predict_var(tmp_dir)
        #---
        ...
        with graph.as_default():
            with tf.Session() as sess:
                ...
                loss, per_example_loss, logits, probabilities = create_classification_model(bert_config=bert_config, is_training=False,
                    input_ids=input_ids, input_mask=input_mask, segment_ids=None, labels=None, num_labels=args.count_labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-count_labels', type=int, default=2,
